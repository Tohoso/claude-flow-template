...
async function main() {
  console.log('=== freee OAuth2 認証 ===\n');

  const adapter = new FreeeAdapter(config.freee);
  ...
  console.log('例: http://localhost:3000/freee/callback?code=XXXXX の XXXXX 部分\n');
  ...
  try {
    ...
    console.log('\n✓ 認証が完了しました！トークンは tokens/freee-token.json に保存されました。');
  } catch (error) {
    console.error('\n✗ 認証に失敗しました:', error);
    process.exit(1);
  }
